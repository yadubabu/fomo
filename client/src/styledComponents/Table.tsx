    ...
    <div className='bg-slate-800 rounded-md m-5 shadow-2xl text-center  pt-5'>
      ...
      <div className="table w-full ... ">
        ...
        <div className={`${showModal}`}>
          <div className='text-end p-1 m-4'>
            <button className='text-black font-bold text-2xl border-red-100' onClick={()=>setShowModal('none')}>X</button>
          </div>
        </div>
        ...
        {coins[0].cap !== null && coins.slice(0,20).map((coin:Coins,index:number=0)=>{
          return(<>
          <hr/>
            <div onClick={()=>setShowModal('display')} key={index} className="tbody table-row ... p-5 m-5 bg-gray-800">
